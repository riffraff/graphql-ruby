...
describe GraphQL::Schema::TimeoutInstrumentation do
  let(:max_seconds) { 1 }
  let(:timeout_instrumentation) {
    GraphQL::Schema::TimeoutInstrumentation.new(max_seconds: max_seconds)
  }
  let(:timeout_schema) {
    ...
    inst = timeout_instrumentation

    GraphQL::Schema.define {
      query(query_type)
      instrument(:field, inst)
    }
  }
  ...
  describe "with a custom block" do
    let(:timeout_instrumentation) {
      GraphQL::Schema::TimeoutInstrumentation.new(max_seconds: max_seconds) do |type, field, ctx|
        raise("Query timed out after 2s: #{ctx.query.operations.count} on #{ctx.ast_node.alias}")
      end
    }
    let(:query_string) {%|
      {
